...
namespace UnityExplorer.Mcp
{
    internal sealed partial class MonoMcpHandlers
    {
        private void AddTools_Components(List<object> list)
        {
            ...
            list.Add(new { name = "ListComponentMembers", description = "List members for a component (fields/properties/methods).", inputSchema = Schema(new Dictionary<string, object> { { "objectId", String() }, { "componentType", String() }, { "includeMethods", Bool(false) }, { "limit", Integer() }, { "offset", Integer() } }, new[] { "objectId", "componentType" }) });
            list.Add(new { name = "ReadComponentMember", description = "Read a component member value (safe).", inputSchema = Schema(new Dictionary<string, object> { { "objectId", String() }, { "componentType", String() }, { "name", String() } }, new[] { "objectId", "componentType", "name" }) });
        ...
        private bool TryCallTool_Components(string key, JObject? args, out object result)
        {
            ...
            switch (key)
            {
                ...
                case "listcomponentmembers":
                    {
                        var oid = RequireString(args, "objectId", "Invalid params: 'objectId' is required.");
                        var ctype = RequireString(args, "componentType", "Invalid params: 'componentType' is required.");
                        result = _tools.ListComponentMembers(oid, ctype, GetBool(args, "includeMethods") ?? false, GetInt(args, "limit"), GetInt(args, "offset"));
                        return true;
                    }
                case "readcomponentmember":
                    {
                        var oid = RequireString(args, "objectId", "Invalid params: 'objectId' is required.");
                        var ctype = RequireString(args, "componentType", "Invalid params: 'componentType' is required.");
                        var name = RequireString(args, "name", "Invalid params: 'name' is required.");
                        result = _tools.ReadComponentMember(oid, ctype, name);
                        return true;
                    }
                case "addcomponent":
                    {
