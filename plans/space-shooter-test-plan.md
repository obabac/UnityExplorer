...
- Mono host: `C:\codex-workspace\space-shooter-build\SpaceShooter_Mono` running at `http://192.168.178.210:51478`; `pwsh ./tools/Run-McpMonoSmoke.ps1 -BaseUrl http://192.168.178.210:51478 -LogCount 10 -StreamLines 3` passes (2 streamed lines including `tool_result`) after switching the harness to a PowerShell HttpClient line reader. Mono now exposes `SpawnTestUi` / `DestroyTestUi` behind `allowWrites+confirm` and `Reparent` / `DestroyObject` gated to those test blocks; use `-EnableWriteSmoke` to enable writes, call SpawnTestUi (returns block ids), reparent a block, destroy a block with `confirm=true`, destroy the UI, and reset config.
