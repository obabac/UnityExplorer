...
- Inspector UI (browser) on the Test‑VM (no manual typing): use `C:\codex-workspace\Start-McpInspectorUi.ps1` to prefill `transport=streamable-http&serverUrl=...` and skip auth typing (`DANGEROUSLY_OMIT_AUTH=true` is set). Examples:
  ...
  - Mono: `pwsh C:\codex-workspace\Start-McpInspectorUi.ps1 -BaseUrl http://192.168.178.210:51478 -LogPath C:\codex-workspace\mcp-inspector-mono-ui.log` (use `-Port 6278` if another inspector instance is already using 6277).
- win-dev control plane (host automation): scheduled tasks `McpProxy8082` (`mcp-proxy` → `desktop-commander.cmd`) and `McpProxy8083` (`mcp-proxy` → `mcp-control.cmd`) listen on `http://192.168.178.210:808{2,3}/mcp`; logs: `C:\codex-workspace\logs\mcp-proxy-808{2,3}.log`. Health check: `curl -I http://192.168.178.210:8082/mcp` / `8083` (200 expected). Requests need `Accept: application/json, text/event-stream` plus the `mcp-session-id` header from `initialize`; methods are `tools/list` + `tools/call` (e.g., `get_config`, `get_screen_size`). Restart with `Start-ScheduledTask -TaskName 'McpProxy8082'/'McpProxy8083'` if down.
