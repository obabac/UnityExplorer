...
- Inspector UX & dev‑experience:
  ...
  - [x] Verify Inspector UI (browser) renders tools/resources cleanly on both hosts (no schema errors; can call `GetStatus`/`TailLogs`, read `unity://status`/`unity://scenes`, and see at least one `stream_events`/SSE notification). IL2CPP UI previously showed empty notifications; SSE channel now streams `tool_result` (covered by contract + inspector CLI); Mono UI pending manual visual check but SSE path is live.
