...
Status (2025-12-12): Space Shooter IL2CPP host unchanged; last contract runs were green (45 passed, 1 skipped placeholder). Mono Space Shooter host is running at `http://192.168.178.210:51478` (`C:\codex-workspace\space-shooter-build\SpaceShooter_Mono`); `pwsh ./tools/Run-McpMonoSmoke.ps1 -BaseUrl http://192.168.178.210:51478 -LogCount 10 -StreamLines 3` now passes and observes `tool_result` after replacing the Python stream parser with a PowerShell HttpClient line reader. `dotnet build src/UnityExplorer.csproj -c ML_Mono` still succeeds (nullable warnings in `McpSimpleHttp`). Inspector schema/UX validation remains pending; console scripts and hooks stay disabled on Mono until validated. Mono smoke wrapper `tools/Run-McpMonoSmoke.ps1` (initialize → list_tools → GetStatus/TailLogs/MousePick → read status/scenes/logs → stream_events tool_result) is available for CI/local Mono hosts.
