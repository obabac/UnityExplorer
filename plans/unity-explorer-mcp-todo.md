...
Status (2025-12-15): win-dev-vm MCP control plane patched so `mcp-proxy` creates a transport when `mcp-session-id` is present on the first `initialize`; after a proxy restart, seed the session id once with `initialize` and the harness tools succeed. Verified `win-dev-vm-command/get_config` (8082) and `win-dev-vm-ui/get_screen_size` (8083) after seeding (logs: `C:\codex-workspace\logs\mcp-proxy-808{2,3}.log`). UnityExplorer MCP game hosts remain up: IL2CPP `http://192.168.178.210:51477` and Mono `http://192.168.178.210:51478` (`/message` returns 200). MousePick parity gap persists (Mono world returns `Items=[]` vs IL2CPP `Items=null`). Contract tests still 55 passed / 1 skipped (`UE_MCP_DISCOVERY=... dotnet test tests/dotnet/UnityExplorer.Mcp.ContractTests -c Release`).
