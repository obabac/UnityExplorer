...
Status (2025-12-15): `tools/Start-McpInspectorUi.ps1` now launches the inspector UI with prefilled `transport=streamable-http&serverUrl=...` (no typing, uses `DANGEROUSLY_OMIT_AUTH=true` and keeps `MCP_PROXY_AUTH_TOKEN`). Ran on the Test-VM for IL2CPP (`:51477`) and Mono (`:51478`); inspector CLI smokes still pass. ML_Mono adds a post-build copy into `Release/.../Mods/` (hashes match after `dotnet build -c ML_Mono`), deployed to `SpaceShooter_Mono`, and `pwsh ./tools/Run-McpMonoSmoke.ps1 -EnableWriteSmoke` now passes end-to-end (spawn/reparent/destroy/reset). CORS + SSE/tool_result coverage remains; contract suite still 54 passed, 1 skipped. Keep launching games via interactive GPU sessions and leave `allowWrites=false` / `requireConfirm=true` when done.
