...
Status (2025-12-15): win-dev-vm MCP control plane alias tools validated after restart by seeding the session with `initialize` (`Accept: application/json, text/event-stream` + `clientInfo`, e.g., `protocolVersion=2024-11-05`, `capabilities={}`); `win-dev-vm-ui/State-Tool` and `win-dev-vm-ui/Powershell-Tool` now return successfully (logs: `C:\codex-workspace\logs\mcp-proxy-808{2,3}.log`). UnityExplorer MCP game hosts remain up: IL2CPP `http://192.168.178.210:51477` and Mono `http://192.168.178.210:51478` (`/message` and `/mcp` return 200). MousePick parity gap persists (Mono world returns `Items=[]` vs IL2CPP `Items=null`). GET `/read` now returns JSON with CORS headers (shared responder); IL2CPP build redeployed, but the contract suite still knocks the IL2CPP host offline (connection reset/refused mid-run). Inspector validation is CLI-only; use `pwsh ./tools/Run-McpInspectorCli.ps1 -BaseUrl <url>` (base with or without `/mcp`) or direct `npx @modelcontextprotocol/inspector --cli` one-liners (see `README-mcp.md`); the Inspector UI helper is deprecated.
