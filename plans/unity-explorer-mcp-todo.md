...
Status (2025-12-14): Streamable-HTTP now serves SSE on `GET /` with `Accept: text/event-stream`, and JSON-RPC notifications without an `id` return HTTP 202 so the inspector client opens the SSE channel. CORS is enabled for browser inspector clients (`Access-Control-Allow-Origin:*`, allow headers/methods, `OPTIONS` preflight=204) and covered by contract tests (preflight, JSON-RPC responses, SSE headers, `GET /read`). SSE + notification coverage landed in the contract suite; `UE_MCP_DISCOVERY=ue-mcp-il2cpp-discovery.json dotnet test tests/dotnet/UnityExplorer.Mcp.ContractTests -c Release` now reports 54 passed, 1 skipped. Inspector CLI smoke passes on IL2CPP (http://192.168.178.210:51477) and Mono (http://192.168.178.210:51478). Browser inspector UI on IL2CPP failed to render resources/notifications this run (UI URL field garbling; no server notifications); Mono UI not exercised yet, so the UI validation checkbox stays open. Mono write smoke currently fails because the Mono tool surface lacks `Reparent`/`DestroyObject` (error `Tool not found: Reparent`); allowWrites reset to false/requireConfirm true. Selection stream parity, MousePick UI ordering, Mono `/read?uri=...` parsing, and smoke harness assertions remain in place; continue launching games via interactive GPU sessions to avoid Microsoft Basic Render Driver (graphics device null) and keep xUnit parallelization disabled. MCP errors still surface as `[MCP]` log lines in the MCP buffer.
...
- Inspector UX & dev‑experience:
  ...
  - [ ] Verify Inspector UI (browser) renders tools/resources cleanly on both hosts (no schema errors; can call `GetStatus`/`TailLogs`, read `unity://status`/`unity://scenes`, and see at least one SSE `tool_result` notification). Current status: inspector CLI + contract tests are green, but IL2CPP browser UI is still unreliable due to URL input garbling; fix the no-typing UI launch (prefill `transport=streamable-http` + `serverUrl=...`) and then re-validate IL2CPP + Mono visually.
