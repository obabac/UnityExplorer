...
Status (2025-12-15): Test-VM validation rerun. IL2CPP 51477 inspector CLI PASS; `UE_MCP_DISCOVERY=ue-mcp-il2cpp-discovery.json dotnet test tests/dotnet/UnityExplorer.Mcp.ContractTests -c Release` → 54 passed, 1 skipped. Mono 51478 inspector CLI PASS; `pwsh ./tools/Run-McpMonoSmoke.ps1 -EnableWriteSmoke` PASS (spawn/reparent/destroy/time-scale path with selection/log/tool_result stream events). `allowWrites=false` / `requireConfirm=true` restored after smoke. `tools/Start-McpInspectorUi.ps1` remains the fastest way to launch the browser inspector with prefilled transport/serverUrl.
