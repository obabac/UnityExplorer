...
Status (2025-12-19): Progress 97% (150/155). Test‑VM hosts are green on both ports (IL2CPP `51477`, Mono `51478`) via inspector CLI, write-enabled smoke, and contract tests (81 total: 80 passed, 1 skipped placeholder). SearchSingletons + `unity://search/singletons` are present on both hosts; Freecam + Clipboard resources and guarded tools are present; `stream_events` emits deterministic `scenes` and `selection` snapshots on open.
