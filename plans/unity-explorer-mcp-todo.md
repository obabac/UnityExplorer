...
Status (2025-12-15): Control plane on 8083 is alive (`node .../mcp-control/build/index.js --sse --port 8083`); curl to `/mcp` blocks waiting for SSE clients. `Start-McpInspectorUi.ps1` launched proxies on localhost:6274/6277 for IL2CPP (51477) and Mono (51478) but the browser UI stayed disconnected (Edge headless shows localhost refused/default forms); no UI tool calls executed yet. IL2CPP/Mono `unity://status` reachable; configs untouched. MousePick parity gap persists (Mono world returns `Items=[]` vs IL2CPP `Items=null`). Contract tests still 55 passed / 1 skipped (`UE_MCP_DISCOVERY=... dotnet test tests/dotnet/UnityExplorer.Mcp.ContractTests -c Release`).
