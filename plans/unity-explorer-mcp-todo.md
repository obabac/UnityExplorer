...
- Inspector UX & dev‑experience:
  ...
  - [ ] Verify Inspector UI (browser) renders tools/resources cleanly on both hosts (no schema errors; can call `GetStatus`/`TailLogs`, read `unity://status`/`unity://scenes`, and see at least one `stream_events` notification).
