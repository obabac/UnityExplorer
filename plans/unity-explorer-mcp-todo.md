...
Status (2025-12-15): Streamable-HTTP now serves SSE on `GET /` with `Accept: text/event-stream`, and JSON-RPC notifications without an `id` return HTTP 202 so the inspector client opens the SSE channel. CORS is now enabled for browser inspector clients (`Access-Control-Allow-Origin:*`, allow headers/methods, `OPTIONS` preflight=204) and covered by contract tests (preflight, JSON-RPC responses, SSE headers). SSE + notification coverage landed in the contract suite; `UE_MCP_DISCOVERY=ue-mcp-il2cpp-discovery.json dotnet test tests/dotnet/UnityExplorer.Mcp.ContractTests -c Release` now reports 53 passed, 1 skipped. Inspector CLI smoke passes on IL2CPP (http://192.168.178.210:51477) and Mono (http://192.168.178.210:51478) after the SSE changes; allowWrites reset to false/requireConfirm true. Selection stream parity, MousePick UI ordering, Mono `/read?uri=...` parsing, and smoke harness assertions remain in place; continue launching games via interactive GPU sessions to avoid Microsoft Basic Render Driver (graphics device null) and keep xUnit parallelization disabled. MCP errors still surface as `[MCP]` log lines in the MCP buffer.
