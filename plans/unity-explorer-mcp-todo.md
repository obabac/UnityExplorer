...
Status (2025-12-15): win-dev-vm MCP control plane alias tools validated after restart by seeding the session with `initialize` (`Accept: application/json, text/event-stream` + `clientInfo`, e.g., `protocolVersion=2024-11-05`, `capabilities={}`); `win-dev-vm-ui/State-Tool` and `win-dev-vm-ui/Powershell-Tool` now return successfully (logs: `C:\codex-workspace\logs\mcp-proxy-808{2,3}.log`). UnityExplorer MCP game hosts remain up: IL2CPP `http://192.168.178.210:51477` and Mono `http://192.168.178.210:51478` (`/message` returns 200). MousePick parity gap persists (Mono world returns `Items=[]` vs IL2CPP `Items=null`). Contract tests still 55 passed / 1 skipped (`UE_MCP_DISCOVERY=... dotnet test tests/dotnet/UnityExplorer.Mcp.ContractTests -c Release`). Inspector UI for IL2CPP launched via `Start-McpInspectorUi.ps1`; headless Edge capture saved to `C:\codex-workspace\inspector-il2cpp-ui.png`, but visible render through win-dev UI still pending for IL2CPP + Mono.
