...
Status (2025-12-14): inspector CLI + Mono smoke (writes: SetConfig → SpawnTestUi (returns block ids) → Reparent → DestroyObject → SelectObject → DestroyTestUi → config reset) + controlled error tail checks now pass on IL2CPP (http://192.168.178.210:51477) and Mono (http://192.168.178.210:51478) after redeploy/restart with the Mono MainThread same-context guard (fixes selection deadlocks), sorted MousePick UI hits (top-most Id), Mono `/read?uri=...` parsing fixed, and smoke harness asserting a `selection` stream event / extra stream lines. Selection stream payload now matches `unity://selection` via a CoreCLR `SelectObject` broadcast and contract coverage on both hosts; SpawnTestUi MousePick(UI) contract locks the primary `Id` to the first/top-most hit. allowWrites reset to false; `initialize.capabilities.experimental.streamEvents` returns `{}`, `resources/list` is live, and `call_tool` returns inspector-friendly content. Space Shooter rebuild automation still succeeds (scene fallback + headless CPU lighting; `Build-SpaceShooter-Remote.ps1` emits logs under `C:\codex-workspace\space-shooter-build\logs`). IL2CPP inspector CLI stays green after the dropdown guard; the Mono smoke run passes. Full IL2CPP contract suite now runs clean (twice) against the IL2CPP host after restarting Space Shooter in an interactive GPU session; earlier crash reproduced when the game launched with Microsoft Basic Render Driver in `Player-prev.log` (Graphics device is null). Keep watching `Player.log`/`Latest.log` for device fallback and restart via interactive session before suites. Added `AssemblyInfo.cs` to disable xUnit parallelization; keep it on. MCP error logging writes `[MCP] error ...` lines into the MCP log buffer on both CoreCLR and Mono. Synced `plans/mcp-interface-concept.md` to the current surfaces and reran the IL2CPP contract suite (`UE_MCP_DISCOVERY=ue-mcp-il2cpp-discovery.json dotnet test tests/dotnet/UnityExplorer.Mcp.ContractTests -c Release`) against http://192.168.178.210:51477 (49 passed, 1 skipped).
...
- Inspector UX & dev‑experience:
  ...
  - [ ] Verify Inspector UI (browser) renders tools/resources cleanly on both hosts (no schema errors; can call `GetStatus`/`TailLogs`, read `unity://status`/`unity://scenes`, and see at least one `stream_events` notification). IL2CPP UI via proxy: resources/tools schemas + GetStatus succeeded; server notifications still empty; Mono UI pending (URL entry glitch).
