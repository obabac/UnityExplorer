...
Status (2025-12-15): Fresh IL2CPP + ML_Mono builds deployed after fixing Mono DTO instantiation in `MonoReadTools` (property initializers for `CameraInfoDto`/`PickResultDto`/`PickHit` so ML_Mono builds cleanly). Test-VM validation rerun: IL2CPP 51477 inspector CLI PASS; `UE_MCP_DISCOVERY=ue-mcp-il2cpp-discovery.json dotnet test tests/dotnet/UnityExplorer.Mcp.ContractTests -c Release` → 55 passed, 1 skipped (log-notification stream_events contract included). Mono 51478 inspector CLI PASS; `pwsh ./tools/Run-McpMonoSmoke.ps1 -EnableWriteSmoke` PASS (SpawnTestUi + SetMember(Image.color) + reparent/destroy/time-scale with selection/log/tool_result events); `allowWrites=false` / `requireConfirm=true` restored after smoke. Mono MousePick UI ordering/primaryId and CameraInfo still match IL2CPP (InputManager pointer coords; world mode omits `Items`); `tools/Start-McpInspectorUi.ps1` remains the fastest way to launch the browser inspector with prefilled transport/serverUrl.
...
- Inspector UX & dev‑experience:
  ...
  - [x] Verify Inspector UI (browser) renders tools/resources cleanly on both hosts (no schema errors; can call `GetStatus`/`TailLogs`, read `unity://status`/`unity://scenes`, and see at least one SSE `tool_result` notification). Done via `tools/Start-McpInspectorUi.ps1` (prefills transport/serverUrl; DANGEROUSLY_OMIT_AUTH=true); launched on Test-VM IL2CPP + Mono, saw stream `tool_result`/log activity and ran inspector CLI smokes for both hosts.
