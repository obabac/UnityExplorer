...
- [ ] Phase B — Read-only MCP surface on Mono
  ...
  - [ ] Fix Space Shooter Mono Unity build automation (implement `BuildCommands.BuildWindows64Mono` or adjust the CLI target) so `C:\codex-workspace\space-shooter-build\SpaceShooter_Mono` can be produced for smoke runs.
