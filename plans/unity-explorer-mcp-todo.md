...
Status (2025-12-15): win-dev-vm MCP control plane restored on 8082/8083 via scheduled tasks `McpProxy8082` (desktop-commander) and `McpProxy8083` (mcp-control) running `mcp-proxy` (logs: `C:\codex-workspace\logs\mcp-proxy-808{2,3}.log`). `curl -I http://192.168.178.210:808{2,3}/mcp` now returns 200; use `Accept: application/json, text/event-stream`, read `mcp-session-id` from `initialize`, and call `tools/list` / `tools/call` (e.g., `get_config`, `get_screen_size`). UnityExplorer MCP game hosts remain up: IL2CPP `http://192.168.178.210:51477` and Mono `http://192.168.178.210:51478` (`/message` returns 200). MousePick parity gap persists (Mono world returns `Items=[]` vs IL2CPP `Items=null`). Contract tests still 55 passed / 1 skipped (`UE_MCP_DISCOVERY=... dotnet test tests/dotnet/UnityExplorer.Mcp.ContractTests -c Release`).
