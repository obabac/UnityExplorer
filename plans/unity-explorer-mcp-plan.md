...
- 2025-12-14 manual validation: rebuilt/deployed IL2CPP + Mono to the Test-VM. Inspector CLI smokes succeeded on both hosts (`http://192.168.178.210:51477` / `:51478`), but the browser inspector UI on IL2CPP could not render resources/notifications (URL field garbling; no server notifications); Mono UI not exercised this run. Mono write smoke currently fails because the Mono tool surface is missing `Reparent`/`DestroyObject` (error `Tool not found: Reparent`); `allowWrites=false` / `requireConfirm=true` were restored via `SetConfig`.
- Browser inspector is unblocked via CORS: all responses include `Access-Control-Allow-Origin:*` + allow headers/methods and `OPTIONS` preflight returns 204; added contract tests covering preflight, JSON-RPC responses, and SSE headers.
- Synced `plans/mcp-interface-concept.md` with SSE + 202 notification behavior; contract suite updated with SSE/tool_result + notification-without-id coverage and CORS headers on `GET /read` for browser inspector: `UE_MCP_DISCOVERY=ue-mcp-il2cpp-discovery.json dotnet test tests/dotnet/UnityExplorer.Mcp.ContractTests -c Release` → 54 passed, 1 skipped (host http://192.168.178.210:51477, Ready=true).
...
- Inspector CLI continues to pass on Test-VM IL2CPP (`http://192.168.178.210:51477`) and Mono (`http://192.168.178.210:51478`) after redeploy/restart; `[MCP] error ...` still surfaces in `unity://logs/tail` after a bad request. Mono write smoke currently fails because `Reparent`/`DestroyObject` are absent from the Mono tool surface; allowWrites/confirm restored to safe defaults.
...
- Mono Space Shooter host (`http://192.168.178.210:51478`) is up: inspector CLI smoke passes (Ready=true, Scenes=1). `pwsh ./tools/Run-McpMonoSmoke.ps1 -EnableWriteSmoke` currently fails at the `Reparent` step (tool not found in Mono build); guarded writes available in Mono today: `SetConfig/SetActive/SelectObject/GetTimeScale/SetTimeScale/SpawnTestUi/DestroyTestUi` gated by allowWrites+confirm.
- IL2CPP dropdown refresh crash is now guarded (warning + continue); `Mods\UeMcpHeadless.dll` is disabled on the Test-VM. IL2CPP host (`http://192.168.178.210:51477`) is back online for inspector CLI; the full contract suite now runs clean (ran twice) after restarting Space Shooter in an interactive GPU session (Player-prev.log earlier showed Microsoft Basic Render Driver -> graphics device null). Keep launching via an interactive session and monitor logs for device fallback; xUnit parallelization stays disabled.
